            ...
            child: Row(
              ...
              children: [
                ...
                ElevatedButton.icon(
                  ...
                  label: Text(
                    AppLocalizations.of(context)!.jobsNewJobCard,
                  ...
                  child: Row(
                    children: filters.map((f) {
                      ...
                      return Padding(
                        ...
                        child: InkWell(
                          onTap: () =>
                              ref.read(jobFilterProvider.notifier).state = f.$1,
          ...
          Expanded(
            child: filteredJobs.isEmpty
                ? EmptyStateView(
                    ...
                    description:
                        'Create a new job card to track service work, assigned mechanics, and vehicle status.',
                                      ...
                                      child: DataTable(
                                        ...
                                        rows: filteredJobs.map((job) {
                                          ...
                                          final initials = customer.name
                                              ...
                                              .join('')
                                              .toUpperCase();
                                          final avatar = initials.isNotEmpty
                                              ? initials
                                              : '?';
                                          return DataRow(
                                            cells: [
          ...
          Expanded(
            child: filteredJobs.isEmpty
                ...
                : ListView.separated(
                    ...
                    itemCount:
                        filteredJobs.length +
                        (ref.watch(jobsListStateProvider).hasMore ? 1 : 0),
                    separatorBuilder: (_, _) => const SizedBox(height: 10),
                    itemBuilder: (ctx, i) {
                      if (i == filteredJobs.length) {
                        return const _LoadMoreJobsButton();
                      }
...
class _LoadMoreJobsButton extends ConsumerWidget {
  const _LoadMoreJobsButton();

  @override
  Widget build(BuildContext context, WidgetRef ref) {
    final jobsState = ref.watch(jobsListStateProvider);
    if (!jobsState.hasMore) return const SizedBox.shrink();
    return Padding(
      padding: const EdgeInsets.only(bottom: 24.0),
      child: Center(
        child: jobsState.isLoadMore
            ? const CircularProgressIndicator()
            : ElevatedButton(
                onPressed: () =>
                    ref.read(jobsListStateProvider.notifier).loadMore(),
                style: ElevatedButton.styleFrom(
                  backgroundColor: kPrimary,
