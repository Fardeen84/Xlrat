// lib/screens/billing/invoice_screen.dart
...
class _BottomActionsState extends State<_BottomActions> {
  ...
  Future<void> _onWhatsApp() async {
    if (_waLoading) return;

    // Phone number check
    final phone = widget.invoice.customer?.mobile ?? '';
    if (phone.isEmpty) {
      _showError('Customer ka phone number nahi hai.');
      return;
    }

    setState(() => _waLoading = true);
    try {
      await InvoicePdfService.sendWhatsApp(widget.invoice);
    } catch (e) {
      if (mounted) {
        _showError('WhatsApp open nahi hua: $e');
      }
    } finally {
        ...
        children: [
          ...
          Expanded(
            child: ElevatedButton(
              onPressed: _pdfLoading ? null : _onDownloadPdf,
          ...
          Expanded(
            child: OutlinedButton(
              onPressed: _waLoading ? null : _onWhatsApp,
